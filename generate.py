...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap:
            ...
            for value_x in self.domains[x]:
                valid_y_values = {
                    value_y for value_y in self.domains[y] if value_x[i] == value_y[j]}
                if not valid_y_values:
                    inconsistent_values.add(value_x)
            for value in inconsistent_values:
        ...
        while queue:
            ...
            if self.revise(x, y):
                if not self.domains[x]:
                    return False
                queue.extend((x, z)
                             for z in self.crossword.neighbors(x) if z != y)
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        values.sort(key=lambda value: self.count_eliminated_values(
    ...
    def count_eliminated_values(self, var, value, assignment):
        """
        Helper function to count the number of values eliminated for neighboring variables
        when assigning `value` to `var` in `assignment`.
        """
        count = 0
    ...
    def select_unassigned_variable(self, assignment):
        ...
        unassigned_variables.sort(key=lambda var: (
